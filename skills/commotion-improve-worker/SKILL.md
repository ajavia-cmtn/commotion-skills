...
| Talked over the caller / long silences | `turnTimeline[]` overlap, `audioMetrics.silenceRatioPercent` / `botRatioPercent` | barge-in / VAD + speaking-plan config, not prompt prose — interruption has **two mutually exclusive modes**, and switching between them means clearing the losing mode's fields, not just its flag | aiworker-lifecycle.md ("Interruption config") |
